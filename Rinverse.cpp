...
unordered_map<int,int> mp={{6,8}, {7,25}};
...
class RMatrix {
    public:
        ...
        double **R;	// Matrix
        double time_take_single_thread;
...
void RMatrix::initialize_matrix (int matrix_size) {
    ...
    time_take_single_thread = execution_time;
...
void RMatrix::compute_matrix_inverse_recursively (double **R, int start, int size) {
    ...
    #pragma omp taskwait
    {
        // cout<<"after taskwait start: "<<start<<" size: "<<size<<endl;
	compute_off_diagonal_block(R, start, size/2);
        // cout<<"after taskwait end start: "<<start<<" size: "<<size<<endl;
...
void RMatrix::compute_off_diagonal_block (double **R, int start, int size) {
    double sum;
    // Rtemp = -inv(R11)*R12
    // omp_set_num_threads((int)(log(size)/log(2)));
...
void RMatrix::invert_upper_triangular_matrix_block (double **R, int start, int size) {
    double sum;
    // cout<<"from invert upper "<<omp_get_thread_num()<<endl;
    // cout<<"inverting "<<omp_get_num_threads()<<endl;
...
//    R.compute_matrix_inverse();
    ...
    printf("Matrix Size = %d,  Leaf Matrix Size = %d, Error = %d, Execution Time(parallel) = %8.4f Execution Time (serial)= %8.4f\n", matrix_size, leaf_matrix_size, error, execution_time, R.time_take_single_thread);
}
